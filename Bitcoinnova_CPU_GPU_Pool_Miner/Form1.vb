...
Public Class Form1
    ...
    Private Sub Form1_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        pool.SelectedItem = pool.Items(0)
        ...
        If updateperm(0) = "y" Then
            ...
            Using client = New WebClient()
                Dim versionnumber As String = client.DownloadString("http://pool.bitcoinn.biz/version.version")
                If versionnumber = "0.2.0" Then
                Else
                    MsgBox("New version available!")
                    Dim webAddress As String = "http://bitcoinn.biz/"
    ...
    Private Sub mining_Click(sender As Object, e As EventArgs) Handles mining.Click
        If wallet_address.Text = "btn_your_wallet_address_here_EAsX15ieXY1NAk9Yu3NoVBfBqUgsFPsv47Ff4W1t491vHiD8fyGHD7nR7gVk1FrcbP2d2mJfBt3M45NgbV6ZRcSdHixcSPH" Then
            wallet_address.Text = "EAsX15ieXY1NAk9Yu3NoVBfBqUgsFPsv47Ff4W1t491vHiD8fyGHD7nR7gVk1FrcbP2d2mJfBt3M45NgbV6ZRcSdHixcSPH"
        ...
        Else
            ...
            If cpuorgpu.SelectedItem = cpuorgpu.Items(1) And gpubrand.SelectedItem = gpubrand.Items(0) And miner_type.SelectedItem = miner_type.Items(1) Then
                Dim objWriter As New System.IO.StreamWriter(FILE_NAME, True)
                objWriter.WriteLine("ccminer -a cryptonight -o stratum+tcp://" & pool_url & ":" & port.Text & " -u " & wallet_address.Text & " -p x -t " & threads.Text & "pause")
                objWriter.Close()
                System.Diagnostics.Process.Start("mine.bat")
                Dim minerstring As String = (globalindex & "    |    " & wallet_address.Text.Substring(0, 40) & "   |   " & miner_type.SelectedItem & "   |   " & pool_url)
                open_miners.Items.Add(minerstring)
    ...
    Private Sub pool_SelectedIndexChanged(sender As Object, e As EventArgs) Handles pool.SelectedIndexChanged
        ...
        If Not (pool.SelectedItem = pool.Items(0) Or pool.SelectedItem = pool.Items(1) Or pool.SelectedItem = pool.Items(2) Or pool.SelectedItem = pool.Items(3) Or pool.SelectedItem = pool.Items(4) Or pool.SelectedItem = pool.Items(5)) Then
        ...
        If pool.SelectedItem = pool.Items(0) Then
            pool_url = "pool.bitcoinn.biz"
            status.Text = status.Text & vbNewLine & ">>pool.bitcoinn.biz selected, 4% Pool fee, 20 BTN Minimum Cashout"
        End If
        If pool.SelectedItem = pool.Items(1) Then
            pool_url = "bitcoinn.multipool.es"
            status.Text = status.Text & vbNewLine & ">>bitcoinn.multipool.es selected, 2% Pool fee, 1 BTN Minimum Cashout"
        End If
        If pool.SelectedItem = pool.Items(2) Then
            pool_url = "bitcoinn.criptominando.eu"
            status.Text = status.Text & vbNewLine & ">>bitcoinn.criptominando.eu selected, 2% Pool fee, 10 BTN Minimum Cashout"
        End If
        If pool.SelectedItem = pool.Items(3) Then
            pool_url = "btn.dreampool.info"
            status.Text = status.Text & vbNewLine & ">>btn.dreampool.info selected, 2% Pool fee, 500 BTN Minimum Cashout"
        End If
        If pool.SelectedItem = pool.Items(4) Then
            pool_url = "superblockchain.con-ip.com"
            status.Text = status.Text & vbNewLine & ">>superblockchain.con-ip.com/btn selected, 2% Pool fee, 20 BTN Minimum Cashout"
        End If
        If pool.SelectedItem = pool.Items(5) Then
            pool_url = "btn.gonspool.com"
            status.Text = status.Text & vbNewLine & ">>btn.gonspool.com, 2% Pool fee, 50 BTN Minimum Cashout"
        End If
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles check.Click
        If pool.SelectedItem = pool.Items(6) Then
            status.Text = status.Text & vbNewLine & "   INFO: Cannot check BTN of custom pool directly from miner, go to their website."
        ElseIf Not (pool.SelectedItem = pool.Items(0) Or pool.SelectedItem = pool.Items(1) Or pool.SelectedItem = pool.Items(2) Or pool.SelectedItem = pool.Items(3) Or pool.SelectedItem = pool.Items(4) Or pool.SelectedItem = pool.Items(5)) Then
            status.Text = status.Text & vbNewLine & "   INFO: Cannot check BTN of custom pool directly from miner, go to their website."
        Else
    ...
    Private Sub donate_Click(sender As Object, e As EventArgs) Handles donate.Click
        Dim webAddress As String = "https://bitcoinn.biz/comunidad-criptomoneda-bitcoin-n.html"
        Process.Start(webAddress)
        wallet_address.Text = "EAsX15ieXY1NAk9Yu3NoVBfBqUgsFPsv47Ff4W1t491vHiD8fyGHD7nR7gVk1FrcbP2d2mJfBt3M45NgbV6ZRcSdHixcSPH"
        status.Text = status.Text & vbNewLine & ">Developer wallet address selected! Make sure you know this before you mine!"
        status.Text = status.Text & vbNewLine & ">Developer wallet address selected! Make sure you know this before you mine!"
